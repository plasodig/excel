...
-- Apply ke remote D1:
--   wrangler d1 execute tutorial-db --remote --file=./migrations/002_requests.sql
-- Apply ke local D1:
--   wrangler d1 execute tutorial-db --local  --file=./migrations/002_requests.sql

CREATE TABLE IF NOT EXISTS generation_requests (
    id                   TEXT PRIMARY KEY,              -- uuid/random
    query                TEXT NOT NULL,                 -- original user query
    slug_target          TEXT NOT NULL,                 -- slugified id untuk tabel excels
    ...
    resulting_excel_id  TEXT REFERENCES excels(id)    -- set saat completed
);
