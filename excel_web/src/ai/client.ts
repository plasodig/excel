...
import type { GeneratedImageBytes, GeneratedExcelContent } from "../types";
import { type AccountSlot, PoolIterator, QuotaExhaustedError } from "./pool";
import { SYSTEM_EXCEL, imagePrompt, userPromptExcel } from "./prompts";
...
export async function generateExcel(
  ...
  title: string,
  categoryHint: string,
): Promise<GeneratedExcelContent> {
  const tryModel = async (model: string) => {
    const body = {
      messages: [
        { role: "system", content: SYSTEM_EXCEL },
        { role: "user", content: userPromptExcel(title, categoryHint) },
      ],
      max_tokens: 1500,
      temperature: 0.3,
    };
...
function parseExcelJson(raw: string): GeneratedExcelContent {
  let text = raw.trim();
