...
publicRoutes.get("/", async (c) => {
  const rows = await listPublishedExcels(c.env.DB);
  return c.html(landingPage(rows));
});

publicRoutes.get("/tutorial", async (c) => {
  const rows = await listPublishedExcels(c.env.DB);
  return c.html(landingPage(rows));
});

publicRoutes.get("/tutorial/:id", async (c) => {
  const id = c.req.param("id");
  const full = await getExcelFull(c.env.DB, id, "");
  if (!full || full.excel.status !== "published") {
    return c.redirect("/tutorial");
  }
  return c.html(excelDetailPage(full));
});

// Manifest ringan: list tutorial published dengan pagination opsional.
...
publicRoutes.get("/api/excels", async (c) => {
  ...
  const total = paged ? await countPublishedExcels(c.env.DB, qCategory) : data.length;
...
publicRoutes.get("/api/excels/:id", async (c) => {
  const id = c.req.param("id");
  const full = await getExcelFull(c.env.DB, id, "");
  if (!full || full.excel.status !== "published") {
...
// A. Suggestions: saran tutorial yang sudah published untuk query user yang miss.
...
// B. Auto-generate: user minta tutorial baru → langsung dispatch AI generation di background.
...
async function handleGenerationSubmit(c: Context<{ Bindings: Bindings }>) {
  ...
  const safety = validateExcelQuery(query);
  if (!safety.ok) {
    return c.json(
  ...
  const existingExcel = await findExcelById(c.env.DB, slug);
  if (existingExcel && existingExcel.status === "published") {
    return c.json({
      success: true,
      status: "already_exists",
      excelId: existingExcel.id,
      message: "Tutorial sudah tersedia di katalog.",
    });
  }
  ...
  if (existingReq) {
    return c.json({
      ...
      message: "Tutorial sedang dibuat. Tunggu sebentar ya.",
    });
  }
  ...
  return c.json(
    {
      ...
      message: "Tutorial sedang dibuat. Biasanya butuh 15-30 detik.",
    },
    202,
  );
}
...
publicRoutes.get("/api/requests/:id", async (c) => {
  ...
  return c.json({
    ...
    excelId: req.resulting_excel_id,
  });
});
...
const VALID_REASONS = new Set([
  "inaccurate", // tutorial tidak akurat / bahan salah
...
publicRoutes.post(
  "/api/excels/:id/report",
  rateLimit({ bucket: "reports", limit: 10, windowSec: 3600 }),
  async (c) => {
    const excelId = c.req.param("id");
    ...
    const excel = await findExcelById(c.env.DB, excelId);
    if (!excel) {
    ...
    return c.json({
      ...
      message: result.unpublished
        ? "Laporan diterima. Tutorial telah otomatis disembunyikan untuk ditinjau."
...
publicRoutes.get("/api/images/excels/:filename", async (c) => {
  const filename = c.req.param("filename");
  const key = `excels/${filename}`;
