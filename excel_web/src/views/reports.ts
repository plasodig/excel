...
import type { ReportWithExcel } from "../db";
import { layout } from "./layout";

export function reportsView(
  rows: ReportWithExcel[],
  flash?: { kind: "ok" | "error"; msg: string },
) {
  const body = html`
    <h1>Laporan Tutorial</h1>
    <p class="small">
              ...
              ${rows.map(
                (r) => html`
                  <tr>
                    ...
                    <td>
                      ${r.excel_title
                        ? html`<a href="/admin/excels/${r.excel_id}">${r.excel_title}</a>`
                        : html`<span class="small">${r.excel_id}</span>`}
                    </td>
                    <td>
                      ${r.excel_status
                        ? html`<span class="badge badge-${r.excel_status}">${r.excel_status}</span>`
                        : "—"}
                    </td>
