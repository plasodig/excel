...
import type { ExcelFull, ExcelRow } from "../types";
...
export function landingPage(rows: ExcelRow[]) {
...
<head>
  ...
  <title>Tutorial Excel Indonesia — Studi Kasus Dunia Kerja</title>
  <meta name="description" content="Koleksi lengkap tutorial Microsoft Excel dengan studi kasus dunia kerja Indonesia: rekap gaji, absensi, stok barang, dan produktivitas kantor." />
...
<body>
  <header class="hero">
    ...
    <div class="hero-content">
      <span class="hero-badge">📈 Tutorial Excel Terlengkap</span>
      <h1>Mastering <span class="gradient-text">Excel</span> Indonesia</h1>
      <p class="hero-subtitle">Jelajahi <strong>${rows.length}</strong> tutorial Excel praktis dengan studi kasus dunia kerja Indonesia</p>
      <div class="hero-search">
        <input type="text" id="searchInput" placeholder="Cari rumus atau kasus... (contoh: VLOOKUP Gaji)" autocomplete="off" />
      </div>
    </div>
  </header>
  ...
  <main>
    ...
    <div class="excel-grid" id="excelGrid">
      ${rows.map(
        (r) => html`
          <a href="/tutorial/${r.id}" class="excel-card" data-cat="${r.category}" data-title="${r.title.toLowerCase()}">
            ...
            <div class="card-body">
              <h3>${r.title}</h3>
              <p class="card-desc">${r.description ? r.description.slice(0, 100) + (r.description.length > 100 ? "…" : "") : "Tutorial Excel praktis Indonesia"}</p>
              <div class="card-meta">
                ${r.cooking_time_minutes ? html`<span>⏱ ${r.cooking_time_minutes} mnt</span>` : ""}
                ${r.servings ? html`<span>📦 Ver. ${r.servings}</span>` : ""}
    ...
    <div class="empty-state" id="emptyState" style="display:none;">
      ...
      <p>Tidak ada tutorial yang cocok dengan pencarian Anda.</p>
    </div>
  </main>
...
export function excelDetailPage(full: ExcelFull) {
  const r = full.excel;
...
<head>
  <meta charset="UTF-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1" />
  <title>${r.title} — Tutorial Nusantara</title>
  <meta name="description" content="${r.description || `Tutorial ${r.title} lengkap dengan bahan dan cara memasak.`}" />
  <link rel="preconnect" href="https://fonts.googleapis.com" />
  <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin />
  <link href="https://fonts.googleapis.com/css2?family=Plus+Jakarta+Sans:wght@400;500;600;700;800&display=swap" rel="stylesheet" />
  <style>${raw(LANDING_CSS)}</style>
...
<body>
  <nav class="detail-nav">
    <a href="/tutorial" class="back-link">← Kembali ke Daftar Tutorial</a>
  </nav>
    ...
    <div class="detail-hero">
      ...
      <div class="detail-hero-info">
        ...
        <div class="detail-stats">
          ${r.cooking_time_minutes ? html`<div class="stat"><span class="stat-val">⏱ ${r.cooking_time_minutes}</span><span class="stat-label">menit belajar</span></div>` : ""}
          ${r.servings ? html`<div class="stat"><span class="stat-val">📦 ${r.servings}</span><span class="stat-label">minimal versi</span></div>` : ""}
    ...
    <div class="detail-content">
      <section class="detail-section">
        ...
        ${full.ingredients.length > 0
          ...
          : html`<p class="muted">Detail rumus belum tersedia.</p>`}
      </section>
 
      <section class="detail-section">
        <h2>👩‍💻 Langkah-Langkah Tutorial</h2>
        ${full.steps.length > 0
          ...
          : html`<p class="muted">Langkah tutorial belum tersedia.</p>`}
      </section>
    </div>
  </article>

  <footer>
    <p>&copy; ${new Date().getFullYear()} Tutorial Nusantara — <a href="https://excel.plasodig.my.id">excel.plasodig.my.id</a></p>
    <div style="margin-top: 12px; display: flex; justify-content: center; gap: 16px;">
      <a href="/privacy-policy">Privacy Policy</a>
      <a href="/contacts">Contact Us</a>
    </div>
...
const LANDING_CSS = `
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  /* ── Excel Grid ── */
  .excel-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(280px, 1fr)); gap: 24px; }
  .excel-card { display: flex; flex-direction: column; background: var(--surface); border: 1px solid var(--border);
                 border-radius: var(--card-radius); overflow: hidden; transition: transform .25s, box-shadow .25s, border-color .25s; }
  .excel-card:hover { transform: translateY(-4px); box-shadow: 0 12px 32px rgba(0,0,0,.35);
  ...
  .excel-card:hover .card-img-wrap img { transform: scale(1.06); }
  ...
  @media (max-width: 768px) {
    ...
    .excel-grid { grid-template-columns: repeat(auto-fill, minmax(240px, 1fr)); gap: 16px; }
  }
`;
...
const LANDING_JS = `
  ...
  const cards = Array.from(grid.querySelectorAll('.excel-card'));
