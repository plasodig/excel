...
//
// Flow:
//   1. Pastikan row `excels` dengan id = slug ada (insert kalau belum ada).
//   2. Generate text (Llama) + image (FLUX) paralel via PoolIterator.
//   3. Simpan konten, upload image ke KV, set status='published'.
//   4. Mark request completed (linked ke excelId).
//
// Kalau di tengah ada error → mark request failed + error_message, excel row tetap draft.
...
import { validateExcelQuery, validateGeneratedContent } from "./contentSafety";
...
export async function processGenerationRequest(
    ...
    const queryCheck = validateExcelQuery(query);
    if (!queryCheck.ok) {
    ...
    const category: Category = "BasicFormula"; // default — admin bisa edit di dashboard

    try {
        // 1. Pastikan row excels ada (sebagai draft kalau baru).
        const existing = await findExcelById(env.DB, slug);
        if (!existing) {
            await env.DB.prepare(
                `INSERT INTO excels (id, title, category, status, updated_at) VALUES (?, ?, ?, 'draft', ?)`,
            )
        ...
        const [text, image] = await Promise.all([
            generateExcel(new PoolIterator(pool), effectiveTitle, effectiveCategory),
        ...
        const imageKey = await uploadExcelImage(env.IMAGES, slug, image);
        ...
        console.log(`[generator=${requestId}] COMPLETED → excel=${slug} published.`);
    } catch (e) {
