...
// masuk akal untuk pencarian tutorial.
...
/**
 * Cari suggestions untuk query user di tabel `excels` (status published).
 *
 ...
 *
 * Lalu tiebreaker: cooking_time ASC (tutorial lebih cepat dulu).
 */
export async function findSuggestions(
  db: D1Database,
  rawQuery: string,
  limit: number = 5,
): Promise<ExcelSuggestion[]> {
  ...
  // Index idx_excels_status mempercepat filter status='published'.
  ...
  const scored: ExcelSuggestion[] = rows.map((r) => {
